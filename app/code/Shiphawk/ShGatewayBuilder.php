...
class ShGatewayBuilder
{
    public static function buildOrdersUrl($gatewayUrl, $apiKey) {
        ...
        return 'https://' . $gatewayHost . '/api/v4/orders?' . http_build_query(['api_key' => $apiKey]);
    }

    public static function buildUserUrl($gatewayUrl, $apiKey) {
        $gatewayHost = self::extractGatewayHost($gatewayUrl);

        return 'https://' . $gatewayHost . '/api/v4/user?' . http_build_query(['api_key' => $apiKey]);
    }

    public static function buildOrderUrl($gatewayUrl, $apiKey, $order) {
        $gatewayHost = self::extractGatewayHost($gatewayUrl);

        return 'https://' . $gatewayHost . '/api/v4/orders/' . $order->getIncrementId() . '/cancelled' . '?' .  http_build_query(['api_key' => $apiKey]);
    }

    public static function buildRatesUrl($gatewayUrl, $apiKey) {
        $gatewayHost = self::extractGatewayHost($gatewayUrl);

        return 'https://' . $gatewayHost . '/api/v4/rates?' . http_build_query(['api_key' => $apiKey]);
    }
