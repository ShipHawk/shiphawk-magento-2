...
class Carrier extends AbstractCarrier implements CarrierInterface
{
    ...
    public function getRates($rateRequest)
    {
        ...
        if ($cached && isset($cached['timestamp']) && (time() - $cached['timestamp']) < self::RATE_CACHE_TTL) {
            $this->mlog("Cache hit for key: {$cacheKey}", 'shiphawk_rates_response.log');
    ...
    protected function _get($jsonRateRequest) {
        ...
        if ($curlError) {
            $this->mlog("Request failed after {$elapsed}s | Error: {$curlError} | URL: {$chUrl}", 'shiphawk_rates_response.log');
            $this->logger->error("ShipHawk rate request failed: {$curlError}");
            return null;
        }

        $arr_res = json_decode($resp);
        $this->mlog("Response time: {$elapsed}s | URL: {$chUrl} | Response: {$resp}", 'shiphawk_rates_response.log');
